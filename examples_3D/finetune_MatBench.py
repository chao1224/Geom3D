...
from Geom3D.datasets import DatasetMatBench, MoleculeDataset3DRadius, MoleculeDataset3DFull, MoleculeDatasetOneAtom, MoleculeDataset3DRemoveCenter
from Geom3D.dataloaders import DataLoaderPeriodicCrystal, DataLoaderGemNetPeriodicCrystal
from Geom3D.models import EGNN, SEGNN, TFN, DimeNet, DimeNetPlusPlus, SchNet, SE3Transformer, SphereNetPeriodic, PaiNN, GemNet, EquiformerEnergyPeriodic
...
def split_ablation(dataset): # 80-20
...
def split(dataset): # 60-20-20
    ...
    Nmols = num_mols
    Ntrain = int(0.6 * num_mols)
    Nvalid = int(0.2 * num_mols)
...
def model_setup():
    if args.model_3d == "SchNet":
        model = SchNet(
            ...
            extra_atom_features=args.extra_atom_features,
        )
    ...
    elif args.model_3d == "coGN":
        from Geom3D.models.coGN import coGN
        model = coGN(
            node_class=node_class,
            emb_dim=args.emb_dim,
            extra_atom_features=args.extra_atom_features,
            num_layer=args.coGN_num_layer,
            distance_cutoff=args.MatBench_cutoff,
        )
        graph_pred_linear = None
...
def train(epoch, device, loader, optimizer):
    ...
    for step, batch in enumerate(L):
        ...
        if graph_pred_linear is not None:
            pred = graph_pred_linear(molecule_3D_repr).squeeze(dim=1)
        else:
            pred = molecule_3D_repr.squeeze(dim=1)
...
if __name__ == "__main__":
    ...
    if args.MatBench_split_mode == "standard":
        train_dataset, valid_dataset, test_dataset = split(dataset)
    else:
        train_dataset, valid_dataset, test_dataset = split_ablation(dataset)
    ...
    if args.lr_scheduler == "None":
        ...
        lr_scheduler = optim.lr_scheduler.StepLR(
            optimizer, step_size=args.lr_decay_step_size, gamma=args.lr_decay_factor, verbose=1
        )
