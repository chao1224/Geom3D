...
from Geom3D.datasets import DatasetGO, DatasetGVP, DatasetGOGearNet
...
def model_setup(device):
    if args.GO_level == "mf":
        num_class = 489
    elif args.GO_level == "bp":
        num_class = 1943
    elif args.GO_level == "cc":
        num_class = 320
...
if __name__ == "__main__":
    ...
    dataset_class = DatasetGO
    if args.model_3d == "GearNet":
        dataset_class = DatasetGOGearNet

    train_dataset = dataset_class(root=data_root, level=args.GO_level, split='train')
    valid_dataset = dataset_class(root=data_root, level=args.GO_level, split='valid')
    test_30_dataset = dataset_class(root=data_root, level=args.GO_level, split='test', percent=0.3)
    test_40_dataset = dataset_class(root=data_root, level=args.GO_level, split='test', percent=0.4)
    test_50_dataset = dataset_class(root=data_root, level=args.GO_level, split='test', percent=0.5)
    test_70_dataset = dataset_class(root=data_root, level=args.GO_level, split='test', percent=0.7)
    test_95_dataset = dataset_class(root=data_root, level=args.GO_level, split='test', percent=0.95)

    if args.model_3d == "GVP":
        data_root = "../data/GO_GVP_" + args.GO_level
    ...
    if args.model_3d in ["GearNet", "GearNet_IEConv"]:
        dataloader_kwargs["collate_fn"] = DatasetGOGearNet.collate_fn
    ...
    model, graph_pred_linear = model_setup(device)
